...
This document provides best practices and testing guidelines for AI agents and contributors working on the Campervan AI Assistant Electron application.
...
describe('KnowledgeService', () => {
  it('should load markdown files from data directory', async () => {
    ...
    const result = await service.loadDocument('models/sprinter-specs.md');
    
    expect(result).toBeDefined();
    expect(result.content).toContain('Mercedes Sprinter');
  });
...
describe('IPC Handlers', () => {
  ...
  it('should handle chat:sendMessage and return response', async () => {
    ...
    const message = { content: 'What is a Sprinter van?' };
...
test.describe('Chat Flow', () => {
  ...
  test('should send a message and receive a response', async () => {
    ...
    await input.fill('Tell me about solar panels for campervans');
    await sendButton.click();
    
    // Wait for response
    const response = window.locator('[data-testid="assistant-message"]').last();
    await expect(response).toBeVisible({ timeout: 30000 });
    await expect(response).toContainText(/solar|panel|power/i);
  });

  test('should display source references when available', async () => {
    const input = window.locator('[data-testid="chat-input"]');
    await input.fill('What maintenance does a Sprinter need?');
